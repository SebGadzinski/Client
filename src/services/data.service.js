...
class DataService {
    ...
    async bookMeeting({categorySlug, serviceSlug, startDate, bookingMessage}) {
        try{
            return await api.post('/data/meeting/book', {
                categorySlug, serviceSlug, startDate, bookingMessage
              });
        }catch(err){
            throw err;
        }
    ...
    async getStatusReports() {
        return await this.call(api.get('/data/admin/getVMStatusReport'), "Could not get vm status data");
    }
    ...
    async downloadVMStatusReport(id){
        ...
        try {
            ...
            let response = await api.post('data/admin/vm/downloadVMStatusReport', formData, {
                headers: {
    ...
    async loadingMoreSubCardHistory(workId, paymentHistoryIndex){
        return await this.call(api.post(`data/work/sub/paymentHistory`, {workId, paymentHistoryIndex}));
    }

    async addCardToSubscription(workId, cardToken){
        return await this.call(api.post(`data/work/sub/paymentMethod/add`, {workId, cardToken}));
    }
    ...
    async gitPullClient() {
        return await this.call(api.post('/vm/gitPullClient', {}, {timeout: 60000}), "Could not pull");
    }

    async resetStats() {
        return await this.call(api.post('/data/resetStats'), "Could not reset Stats");
    }
    ...
    async getWorkTemplateEditorPageData({workTemplateId, isNew = false}) {
        return await this.call(api.post('/data/getWorkTemplateEditorPageData', { workTemplateId, isNew}), "Could not get work editor page data");
    }
    async getWorkTemplatePageData() {
        return await this.call(api.get('/data/getWorkTemplatePageData'), "Could not get work template page data");
    }
    async getWorkTemplateComponent(workTemplateId) {
        return await this.call(api.post('/data/work/template',
        { id: workTemplateId }), "Could not get work template component");
    }
    async deleteWorkTemplate(id) {
        return await this.call(api.delete(`/data/work/template/delete/${id}`), "Could not delete template");
    }
    async enrollmentStatus(templateId) {
        return await this.call(api.post(`/data/work/class/enroll/status/${templateId}`));
    }
    async enrollInClass(templateId) {
        return await this.call(api.post(`/data/work/class/enroll/${templateId}`), "Could not enroll.");
    }
    async useSingleSession(workId) {
        return await this.call(api.post(`/data/work/class/use/single-session/${workId}`), "Could not use session.");
    }
    async deleteCard(workId) {
        return await this.call(api.delete(`/data/work/sub/paymentMethod/delete/${workId}`));
    }
